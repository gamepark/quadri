...
| Mode découverte : retirer les cartes valeur 8 | `options.discoveryMode` filtre `objectiveValues[id] < 8` | ✅ |
...
| N'importe quel joueur peut scorer à tout moment | `SimultaneousRule` dans `CheckObjectivesRule` — mais uniquement après chaque pose, pas en cours de tour | ⚠️ |
...
| Règles | Code | Statut |
|--------|------|--------|
| Le dernier avec un objectif en main a gagné | `playersWithObjectives.length <= 1` → `endGame()` ; `getScore = length > 0 ? 1 : 0` | ✅ |
...
|----------|-----------|--------|
| Mode avancé compétitif | *"Retirer les cartes valeur 3"* | ✅ `advancedMode` : `objectiveValues[id] > 3` |
| Coop avancée | *"Ajouter les cartes valeur 6 puis valeur 8"* | ✅ `advancedCooperative` : `objectiveValues[id] > 3` — un seul niveau au lieu des deux paliers PDF |
